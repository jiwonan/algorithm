...
### 출력 
 ...
 ### 풀이 요약
 온기와 수분을 max만큼만 줘야한다는 조건이 없음.
 처음 한 번에 무조건 많이 채워두어야 이득임. (최소값이 0이기 때문에)

 따라서 둘중 더 큰 값에 초과하여 온기나 수분을 채워둔 뒤, 나머지 값을 채운다면 while문을 돌지 않아도 풀 수 있음.

 => 단순하게 생각해야하는 문제인 것 같은데,, 이런걸 어떻게 생각해내야하는지..
