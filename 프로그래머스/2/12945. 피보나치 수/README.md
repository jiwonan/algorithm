...
### 풀이 요약
n+1 만큼의 배열(fibo)에 피보나치 결과를 저장한다. 위 예시에서 나온 것 처럼 0, 1, 1, 2, 3, 5, ... 순서로 저장 후<br/>
fibo[n] % 1234567을 반환하면 될 것이라고 생각했지만 테스트케이스 7번부터 다 틀렸다.<br/>
answer의 타입이 int이기 때문에 int max 값을 넘어가니 연산 결과가 되려 작아지는 현상을 발견했다. (n=1000보다 n=2000의 결과값이 더 작음)<br/>
피보나치 값의 나머지를 배열에 저장한 뒤 반복문을 도니 해당 문제가 해결되었다. (어차피 n의 max는 100,000이기 때문에 나머지가 int max 값을 넘을 수 없음.)
